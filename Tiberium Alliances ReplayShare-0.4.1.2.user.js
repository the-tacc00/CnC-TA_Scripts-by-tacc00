...
// @author         petui
// @contributor    leo7044 (https://github.com/leo7044) - tacc00
// @description    Share combat reports with your friends in other alliances and worlds / game version: 25.2.1 / Last maintenance 24 March 2026
            ...
            qx.Class.define('ReplayShare', {
                ...
                members: {
                    ...
                    initializeHacks: function() {
                        if (ClientLib.Vis.Battleground.Battleground.prototype.LoadCombatDirect === undefined) {
                            ...
                            var loadCombatDirectMethodName = ClientLib.API.Battleground.prototype[onSimulateBattleMethodName].toString()
                                .match(/\$I\.[A-Z]{6}\.[A-Z]{6}\(\)\.[A-Z]{6}\(\)\.([A-Z]{6})\(t\.d\)/)[1];
                    ...
                    handleLink: function(url) {
                        var matches = url.match(/^https?:\/\/ccta\.site\/replayshare\?id=([A-Za-z0-9]+)/) ||
                            url.match(/^https?:\/\/replayshare-api\.cnc-ta-tacc00\.workers\.dev\/([A-Za-z0-9_-]+)/i) ||
                            url.match(/[?#&]replayshare(?:=|%3D)([A-Za-z0-9_-]+)/i);
                        //var matches = url.match(/^https?:\/\/peteststorage\.my\.salesforce\-sites\.com\/apex\/ReplayShare\?id=([A-Za-z0-9]+)/);
                    ...
                    handleCurrentLocation: function() {
                        ...
                        if (this.handleLink(href)) {
                            if (window.location.hash && /replayshare=/i.test(window.location.hash)) {
                ...
                members: {
                    ...
                    formatLinkUrl: function scope(id) {
                        if (scope.baseUrl === undefined) {
                            scope.baseUrl = 'https://replayshare-api.cnc-ta-tacc00.workers.dev/';
                        }

                        return scope.baseUrl + encodeURIComponent(id);
                    },
            ...
            qx.Class.define('ReplayShare.Service', {
                ...
                members: {
                    ...
                    get: function(id, options) {
                        ...
                        this._sendRequest(this.getApiBaseUrl() + '/api/replay/' + encodeURIComponent(id), requestOptions);
                    },
                    ...
                    save: function(replay, options) {
                        ...
                        if (options.success !== undefined) {
                            requestOptions.success = function(data) {
                                var payload = (typeof data === 'string') ? JSON.parse(data) : data;
                                options.success(replay.setId(payload.id));
                            };
                        }
                        ...
                        this._sendRequest(this.getApiBaseUrl() + '/api/replay', requestOptions);
                    },

                    getApiBaseUrl: function() {
                        return 'https://replayshare-api.cnc-ta-tacc00.workers.dev';
                    },
                    ...
                    _sendRequest: function(url, options) {
                        ...
                        if (options.error !== undefined) {
                            request.addListener('error', function(event) {
                                options.error('Network error while contacting ReplayShare API');
                            });
                            ...
                            request.addListener('statusError', function(event) {
                                ...
                                if (response && response.error !== undefined) {
                                    options.error(response.error.message || response.error.statusText || response.error);
                                }
                                else {
